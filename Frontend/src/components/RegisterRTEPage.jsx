...
const RegisterRTEPage = () => {
  ...
  // Generate a random 4-digit number for RTE ID
  const generateRandomRteId = () => {
    const randomId = Math.floor(1000 + Math.random() * 9000);
    return randomId; // Return just the number, not a string with "RTE" prefix
  };

  // Update email and RTE ID when department or names change
  useEffect(() => {
    if (department) {
      setRteId(generateRandomRteId());
  ...
  const handleSubmit = async (e) => {
    ...
    // Dynamically set the RTE data
    const rteData = {
      rte_id: rteId, // This will now be a numeric ID
      email: email,
    ...
    try {
      const response = await fetch("http://localhost:3000/api/rte_post", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(rteData),
      });
            ...
            <div className="mb-4">
              ...
              <div className="relative w-full">
                ...
                >
                  ...
                  <option value="Timetable Management">
...
export default RegisterRTEPage;
